...
class MainScreenFragment : Fragment(R.layout.fragment_main_screen) {
    ...
    private fun connect() {
        ...
        if (video == null || audio == null) {
            toast(R.string.xtream_fill_all); return
        }
    ...
    private fun authenticateAndLoad(a: XtreamAccount): List<StreamItem> {
        ...
        lc.connectTimeout = 20000; lc.readTimeout = 30000
        val liveBody = lc.inputStream.bufferedReader().use { it.readText() }; lc.disconnect()
    ...
    private fun pick(title: Int, items: List<StreamItem>, done: (StreamItem) -> Unit) {
        ...
        val box = LinearLayout(ctx).apply { orientation = LinearLayout.VERTICAL; setPadding(24, 8, 24, 8); addView(search); addView(list, LinearLayout.LayoutParams(-1, 0, 1f)) }
    ...
    private fun clearSaved() {
        PreferenceManager.getDefaultSharedPreferences(requireContext()).edit()
            ...
            .remove(KEY_A_SERVER).remove(KEY_A_USER).remove(KEY_A_PASS).putBoolean(KEY_REMEMBER, false).apply()
    }

    private fun normalizeServer(raw: String): String? = try {
